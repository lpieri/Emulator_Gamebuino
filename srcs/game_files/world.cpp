...
/*   Updated: 2019/01/15 22:03:08 by delay            ###   ########.fr       */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
Image le101 = Image("./assets/le_101_macos.png");
...
void	World::check_end(void)
{
	...
	if (P(character_foot) >= MACOS_HEIGHT || (P(bad) &&
				P(character_foot) >= P(the_bad)))
	{
...
void	World::start_game(void)
{
	...
	gb.display.drawImage(centerX, centerY, le101, P(S_LE101), P(S_LE101));
/*	while (iloop)
	{
		while (!gb.update()){
			if (gb.buttons.pressed(BUTTON_A))
